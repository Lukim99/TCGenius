(function () {
    ...
    class FieldRenderer {
        ...
        recoveryBurst(event) {
            ...
            for(let i=0;i<(reducedMotion?12:34);i++){const a=Math.random()*Math.PI*2,s=.025+Math.random()*.12;this.particle({x:.25+(Math.random()-.5)*.07,y:.65+Math.random()*.12,vx:Math.cos(a)*s,vy:-.08-Math.random()*.18,life:.65+Math.random()*.7,maxLife:1.35,width:.007+Math.random()*.012,height:.007+Math.random()*.018,spin:(Math.random()-.5)*4,color:colors[i%colors.length],mode:i%4===0?2:1});}
        }
        render(time) {
    ...
    class Hud {
        constructor(canvas) {
            this.canvas=canvas;this.ctx=canvas.getContext('2d');this.regions=[];this.images=new Map();this.flashUntil=0;this.flashDuration=1;this.shakeUntil=0;this.banner=null;this.bannerSub='';this.bannerUntil=0;this.damagePops=[];this.rewardStarted=0;this.bossLagHp=null;this.bossKey='';this.entryTransition=null;this.consumableMenu=false;this.consumablePage=0;this.frame=requestAnimationFrame(()=>this.draw());
        ...
        consumableLauncher(w,h){if(!state||!state.inField)return;const c=this.ctx,narrow=w<650,pw=narrow?Math.min(238,w*.61):304,size=narrow?52:58,x=Math.min(w-size-(narrow?12:22),(narrow?12:24)+pw+8),y=h-(narrow?174:166),list=state.consumables||[],disabled=list.length===0;this.cutPath(x,y,size,size,7);const g=c.createLinearGradient(x,y,x,y+size);g.addColorStop(0,disabled?'#24282e':'#253b35');g.addColorStop(1,disabled?'#111318':'#0b211b');c.fillStyle=g;c.fill();c.strokeStyle=disabled?'rgba(100,105,115,.45)':'rgba(98,225,157,.72)';c.lineWidth=1;c.stroke();if(!disabled)this.line(x+7,y+1,x+size*.64,y+1,'rgba(146,255,197,.78)',2);const item=list[0],art=narrow?31:35,ax=x+(size-art)/2,ay=y+5,frame=item&&this.image(item.frameUrl),icon=item&&this.image(item.iconUrl);if(frame)c.drawImage(frame,ax,ay,art,art);if(icon)c.drawImage(icon,ax,ay,art,art);if(!icon){this.line(x+size/2-7,y+19,x+size/2+7,y+19,disabled?'#666b73':'#85e4af',3);this.line(x+size/2,y+12,x+size/2,y+26,disabled?'#666b73':'#85e4af',3);}this.text('회복',x+size/2,y+size-9,8,900,'center',disabled?'#686d75':'#dffbea');this.text('P',x+5,y+7,7,900,'left',disabled?'#60656c':'#a8d8bd');if(list.length)this.text(number(list.reduce((sum,entry)=>sum+Number(entry.count||0),0)),x+size-5,y+size-7,7,900,'right','#8de8b4');if(!disabled&&!busy)this.region(x,y,size,size,()=>this.toggleConsumables());}
        ...
        addRecovery(event){const now=performance.now(),hp=Number(event&&event.recoveredHp||0),mp=Number(event&&event.recoveredMp||0);if(hp>0)this.damagePops.push({damage:hp,recovery:true,resource:'HP',start:now,x:.25,y:.56});if(mp>0)this.damagePops.push({damage:mp,recovery:true,resource:'MP',start:now+(hp>0?120:0),x:.25,y:.61});}
        damageLayer(w,h){const now=performance.now();this.damagePops=this.damagePops.filter(pop=>{const p=(now-pop.start)/950;if(p<0)return true;if(p>=1)return false;const ease=1-Math.pow(1-clamp(p,0,1),3),x=w*pop.x,y=h*pop.y-ease*(pop.received?45:pop.recovery?60:82),alpha=clamp(Math.min(p*8,(1-p)*3.4),0,1),scale=.7+Math.sin(Math.min(1,p)*Math.PI)*.36;this.ctx.save();this.ctx.globalAlpha=alpha;this.ctx.translate(x,y);this.ctx.scale(scale,scale);if(pop.recovery)this.text(pop.resource+' +'+number(pop.damage),0,0,w<650?22:28,900,'center',pop.resource==='HP'?'#63f29b':'#65b9ff');else{if(pop.critical)this.text('치명타',0,-26,w<650?10:12,900,'center','#ffb0a8');this.text((pop.received?'-':'')+number(pop.damage),0,0,pop.received?(w<650?24:30):pop.critical?(w<650?39:53):(w<650?30:40),900,'center',pop.received?'#ff8f87':pop.critical?'#ff5d4d':'#ffe06e');if(pop.skill)this.text(pop.skill,0,25,w<650?9:11,800,'center','#efe4cd');}this.ctx.restore();return true;});}
    ...
    function applyState(next,options) { if(!next)return;state=next;clockOffset=Number(next.serverNow||Date.now())-Date.now();if(renderer)renderer.setState(next);if(!next.inField)hud.closeConsumables();if(next.inField&&!(options&&options.deferBgm))audio.playBgm();else audio.stopBgm(); }
