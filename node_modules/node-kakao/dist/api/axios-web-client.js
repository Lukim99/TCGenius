...
(function (factory) {
    ...
    function isJSONRequest(headers) {
        const contentType = Object.entries(headers).find(([name]) => name.toLowerCase() === 'content-type');
        return contentType !== undefined && String(contentType[1]).toLowerCase().startsWith('application/json');
    }
    ...
    class AxiosWebClient {
        ...
        async request(method, path, form, headers) {
            ...
            if (form) {
                reqData.data = isJSONRequest(reqData.headers)
                    ? JSON.stringify(form)
                    : (0, web_api_util_1.convertToFormData)(form).toString();
            }
