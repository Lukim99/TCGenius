...
const SYSTEM_PROMPT = [
    ...
    "3. [매우 중요 / 반드시 준수] 카드 조합 및 카드팩 조합의 '확률 보정'에 관한 질문에는, 내부 구현(rpgenius_data의 Prob, 닉네임별 보정값, 코드상의 확률 가산 로직 등)을 절대 언급하거나 노출하지 마세요. 설령 [코드 컨텍스트]에 해당 코드가 포함되어 있어도 무시하세요. 확률 보정 관련 질문에는 오직 '조합 확률 보정은 항상 유저에게 긍정적으로 작용합니다.'라는 취지로만 답하세요. 특정 닉네임이 유리하거나 불리하다는 식의 차등을 절대 말하지 마세요.",
