...
이벤트 그룹 첫 번째 메뉴에 `윷놀이`를 제공한다. 모든 로그인 계정이 화면과 조회/던지기 API를 이용할 수 있다. 비로그인 요청은 기존 `requireUser` 인증으로 차단한다.
...
`node scripts/init_songpyeon_item.js --apply`를 명시적으로 실행해 없던 송편만 운영 Item ID 387에 추가했다. 기존 387개 아이템이 동일함을 재조회로 확인했다. Bundle/Pack/제작식이나 기존 보상 정의는 수정하지 않았다. 송편 사용 및 사냥 드랍 기능은 새 서버 코드 배포 후 적용된다.
...
100일 캡슐 메뉴·화면·뽑기 구현을 제거했다. 기존 조회/뽑기 API는 410과 이벤트 종료 메시지를 반환하며 코인을 소모하지 않는다. 기존 코인 보유량과 캡슐 운영 데이터는 보존한다. 이전 캡슐 링크는 현재 이벤트 화면으로 이동한다.

사냥의 100일 기념 코인 드랍을 송편으로 교체했다. 기존 확률(기본 5%, 아이템 드랍 및 레벨 보정)과 드랍 가능한 사냥 조건을 유지하며, 1회 당첨 시 1개를 지급한다. 한국 시간 자정 기준 하루 최대 10개이며 `songpyeonDropDaily`에 별도로 기록해 기존 코인 획득 횟수와 섞이지 않는다. 송편 사용이나 관리자 지급은 사냥 획득 한도에 포함되지 않는다. 윷놀이는 모든 로그인 유저에게 공개한다.

윷 아이콘의 CSS 70% 축소는 롤백했다. 기존 아이템 이미지 332개(이벤트·가챠·미끼·번들·보주·사용·소모품·재료·티켓·화폐)의 알파값 16 초과 영역을 비교했으며, 긴 변의 평균 점유율은 46.03%, 기존 윷은 90.67%였다. 이 평균을 목표로 이미지 자체에 투명 여백을 늘린 편집본을 내장 image_gen으로 만들고 `tcgenius/assets/itemImage/이벤트/윷.png`에 재업로드했다. 로컬 작업 사본도 교체했다. 테스트·배포는 실행하지 않았다.

## 전체 공개 및 배너 이동 대상

관리자 배너 설정의 이동 대상에 `이벤트 · 윷놀이`를 추가했다. 기존 `캡슐` 대상은 배너를 읽을 때 `윷놀이`로 해석해 기존 배너도 연결되며, 다음 관리자 저장 시 새 대상으로 저장된다. 운영 배너 목록을 일괄 덮어쓰지는 않았다. 기존 캡슐 탭 링크도 윷놀이로 연결한다.

HTML, 이벤트 메뉴, 탭 로딩 및 두 API의 관리자 제한을 해제했다. 로그인 인증과 현재 유저 조회, 중복 요청 방지·아이템 차감·보상 저장 규칙은 유지한다. 기존 API 테스트의 기대 결과를 일반 유저 이용 가능으로 수정했으며 테스트 실행 및 코드 배포는 하지 않았다.
