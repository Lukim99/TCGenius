...
assert.strictEqual(
    isDcWriteSuccess('<meta http-equiv="refresh" content="0; url=https://m.dcinside.com/board/ai_utilize">'),
    true
);
assert.strictEqual(
    isDcWriteSuccess('<meta http-equiv="refresh" content="0; url=https://example.com/board/ai_utilize">'),
    false
);
assert.strictEqual(
    isDcWriteSuccess('<meta http-equiv="refresh" content="0; url=https://m.dcinside.com/write/ai_utilize">'),
    false
);
...
assert.ok(writeSource.includes('options?.expectedLinkUrl || options?.ogLinkUrl'), 'OG 적용 여부와 별개로 검증할 외부 링크를 지원해야 한다.');
...
assert.ok(writeSource.includes("params.set('use_gall_nickname', '0')"), '사용자 지정 유동닉은 갤러리 기본 닉네임 사용을 해제해야 한다.');
assert.ok(writeSource.includes('10000 + Math.floor(Math.random() * 10001)'), '익명 게시마다 새 sticky 프록시 포트를 선택해야 한다.');
assert.ok(writeSource.includes('createDcStickyAgent(isGuestPost ? guestProxyPort : 10000)'), '익명 게시 과정에서는 선택한 sticky IP를 유지해야 한다.');
